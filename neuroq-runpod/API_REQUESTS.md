...
- `temperature` - ランダム性 0.0-2.0 (デフォルト: 0.8) ※後方互換性
- `temp_min` - 温度の下限 (デフォルト: 0.4) ※推奨
- `temp_max` - 温度の上限 (デフォルト: 0.8) ※推奨
- `top_k` - 上位K個から選択 (デフォルト: 50, layeredのみ)
- `top_p` - 累積確率p (デフォルト: 0.9, layeredのみ)
- `pretrain` - 事前学習 (デフォルト: true)

**温度パラメータについて:**
- `temperature`を指定した場合、自動的に `temp_min = temperature * 0.8`, `temp_max = temperature * 1.2` に変換されます
- より精密な制御が必要な場合は、`temp_min`と`temp_max`を直接指定してください
- 量子的なゆらぎを維持するため、温度を範囲で指定することを推奨します
