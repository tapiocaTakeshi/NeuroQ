...
class NeuroQInference:
        ...
        try:
            ...
            if self.model_configs_available and self.NeuroQuantum is not None:
                ...
                if os.path.exists(checkpoint_path):
                    ...
                    print(f"   📂 /model_checkpoints 内のファイル:")
                    import glob
                    for f in glob.glob("/model_checkpoints/*"):
                        print(f"      {f}")
                    print("🔄 チェックポイントなしで新規モデルを初期化します...")
                    try:
                        config = self.get_model_config(model_size)
                        nq_config = self.NeuroQuantumConfig()
                        ...
                        if os.path.exists(tokenizer_path):
                            ...
                            print(f"   ✅ トークナイザーをロード (vocab_size={config['vocab_size']})")

                        total_params = sum(p.numel() for p in self.model.parameters())
                        print(f"✅ {config['name']} 新規初期化完了 ({total_params:,}パラメータ)")
                        print("   ⚠️ 未学習モデルです。/train エンドポイントで学習してください。")
                    except Exception as e:
                        import traceback
                        print(f"❌ 新規モデル初期化失敗: {e}")
                        traceback.print_exc()
                        return False
    ...
    def _generate_text(self, prompt: str, max_length: int = 50,
        ...
        if self.model is None or not self.is_initialized:
            return {"generated": "Error: モデルが読み込まれていません。チェックポイントを確認してください。/train エンドポイントで学習するか、チェックポイントファイルをアップロードしてください。", "rag_used": False}
    ...
    def get_embeddings(self, text: str, model_size: str = "micro") -> Dict[str, Any]:
        ...
        if self.current_model_size != model_size:
            if not self._load_model(model_size):
                return {
                    "status": "error",
                    "error": f"モデルの読み込みに失敗しました ({model_size})。チェックポイントを確認してください。"
                }
